...
│   ├── generate_markdown_report.py # Markdown + ASCII 报告
│   └── generate_html_report.py     # 交互式 HTML 报告
├── damn.md                         # 可选：与 SKILL 并行的长版说明（.gitignore）
├── evals/
│   └── evals.json       # 测试用例定义
└── README.md
```
...
`python3 scripts/generate_html_report.py -i references/report-data.example.json -o ./damn_report.html`（依赖以脚本内说明为准）。

### 项目内 / 手动

1. **Skill（自动触发）**：将含 `SKILL.md` 的本目录（`damn/`，skill 名称为 `damn`）放入各工具的全局 skills 目录，或放入项目的 `.cursor/skills/` / `.claude/skills/`。
